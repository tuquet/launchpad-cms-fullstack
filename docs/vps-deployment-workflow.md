...
```
*(Script này tự động copy `.env.example` thành `.env`, tạo mã bảo mật ngẫu nhiên, và set `COMPOSE_FILE=compose.prod.yml` để không cần flag `-f` khi gõ lệnh docker compose)*
...
Nhờ biến `COMPOSE_FILE` đã được set tự động, bạn chỉ cần gõ:
```bash
